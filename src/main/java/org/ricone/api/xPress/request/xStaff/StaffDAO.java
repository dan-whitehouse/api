...
public class StaffDAO extends AbstractDAO<Integer, Staff> implements IStaffDAO {
    ...
    public List<StaffWrapper> findAllByCourseRefId(MetaData metaData, String refId) throws Exception {
        ...
        select.distinct(true);
        select.select(cb.construct(StaffWrapper.class, lea.get("leaId"), from));
        select.where(cb.and(cb.equal(course.get("courseRefId"), refId), lea.get(MetaData.LEA_LOCAL_ID_KEY).in(metaData.getApp().getDistrictLocalIds())));
        select.orderBy(cb.asc(from.get(PRIMARY_KEY)));
    ...
    public StaffWrapper findByLocalId(MetaData metaData, String localId) throws NotFoundException {
        ...
        select.distinct(true);
        select.select(cb.construct(StaffWrapper.class, lea.get("leaId"), from));
        select.where(cb.and(cb.equal(staffIdentifiers.get(ID_KEY), localId), cb.equal(staffIdentifiers.get(IDENTIFICATION_SYSTEM_CODE), "District"), lea.get(MetaData.LEA_LOCAL_ID_KEY).in(metaData.getApp().getDistrictLocalIds())));
    ...
    public StaffWrapper findByStateId(MetaData metaData, String stateId) throws Exception {
        ...
        select.distinct(true);
        select.select(cb.construct(StaffWrapper.class, lea.get("leaId"), from));
        select.where(cb.and(cb.equal(staffIdentifiers.get(ID_KEY), stateId), cb.equal(staffIdentifiers.get(IDENTIFICATION_SYSTEM_CODE), "State"), lea.get(MetaData.LEA_LOCAL_ID_KEY).in(metaData.getApp().getDistrictLocalIds())));
